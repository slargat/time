...
} from './get-resize-props'
import type { ResizeConstraints, ResizeEdge } from './get-resize-props'
import type {
...
/** Input to a {@link ResizeHost.validateResize} call (was god-class `./calendar`). */
...
> {
  ...
  private _maybeEmitError(
    ...
    // ponytail: dropped the devtools `getTimeClient().emit` — the onResizeError
    // callback is the public hook. Re-add an emit if devtools wire up later.
