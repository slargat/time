...
/**
 * Resource availability + capacity logic. Salvaged verbatim from
 * `@tanstack/time`'s `availability.ts` (ADR 0007: copied in, self-contained).
 * All state (resources + memo caches + the live event set) is passed via
...
>(
  ...
  for (const resource of resources) {
    ...
    const info = getResourceDayAvail(
      resource,
      weekday,
      ctx.resourceDayAvailCache,
    )
...
>(
  date: string,
  options:
    | { containerHeight?: number; resourceIds?: Array<TResource['id']> }
    | undefined,
  ctx: AvailabilityContext<TResource, TEvent>,
