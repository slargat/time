...
const monthAtom = createAtom<CalendarState["month"]>(
    new Date().getMonth() as CalendarState["month"]
);
const yearAtom = createAtom<CalendarState["year"]>(new Date().getFullYear());
            ...
            <PopoverContent className="w-40 p-0">
                <Command
                    filter={(_value, search) => {
                        const label = data.find((item) => item.value === _value)?.label;
...
export const CalendarBody = ({ features, children, className }: CalendarBodyProps) => {
    ...
    const featuresByDay = useMemo(() => {
        const result: { [day: number]: Array<Feature> } = {};
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        days.push(
            <div
                className={cn("relative flex h-full w-full flex-col gap-1 p-1 text-muted-foreground text-xs", className)}
                key={day}
            >
                {day}
                <div>
...
export const CalendarMonthPicker = ({
    ...
    const monthData = useMemo(() => {
        return monthsForLocale(locale).map((_month, index) => ({
...
export const CalendarProvider = ({
    ...
    <CalendarContext.Provider value={{ locale, startDay }}>
        <div className={cn("relative flex flex-1 min-h-0 flex-col", className)}>{children}</div>
